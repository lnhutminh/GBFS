...
def find_next(coordinates, maze):
    next_coordinates = []
    if coordinates[0] - 1 >= 0 and maze[coordinates[0] - 1][coordinates[1]] != 'x':
        next_coordinates.append((coordinates[0] - 1, coordinates[1]))
    if coordinates[0] + 1 < len(maze) and maze[coordinates[0] + 1][coordinates[1]] != 'x':
        next_coordinates.append((coordinates[0] + 1, coordinates[1]))
    if coordinates[1] + 1 < len(maze[0]) and maze[coordinates[0]][coordinates[1] + 1] != 'x':
        next_coordinates.append((coordinates[0], coordinates[1] + 1))
    if coordinates[1] - 1 >= 0 and maze[coordinates[0]][coordinates[1] - 1] != 'x':
        next_coordinates.append((coordinates[0], coordinates[1] - 1))
    return next_coordinates
def dfs(maze, stack, visited, path):
...
def gbfs(start,n,path):
    ...
    if (temp_idx == goal_index):
        ...
        while to_be_expanded:
            ...
            print('Times:', temp_count, ': Popping', (x2,y2))
            path.append(tuple((x2,y2)))

            
            if (new_idx == goal_index): # If goal state is found, print
                return current_node
            else:
...
# def routeDFS(route, maze):
#     stack = []
#     pathdfs = set()
#     dfs(maze, stack, pathdfs)
#     print(len(pathdfs) - 2)
#     route = list(pathdfs)
#     route.sort(key=lambda tup: (tup[0], tup[1]), reverse=True)
#     print(route)
#     return route

def routeGBFS(route, maze):
    result = gbfs(maze,len(maze[0]),route)
    return route
...
def visualize_maze(matrix, bonus, start, end, route=None):
    ...
    #GBFS
    routeGBFS(route, maze)

    #DFS
    # stack = []
    # pathdfs = set()
    # dfs(maze, stack, pathdfs, route)
